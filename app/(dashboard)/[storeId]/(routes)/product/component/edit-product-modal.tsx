...
export const EditProductModal = React.forwardRef(({
    colorList,
    categoryList,
    sizeList,
}: {
        ...
        return {
            open: (recordData: any) => {
                ...
                createProductForm.setFieldsValue({
                        ...recordData, 
                        images: recordData.images.map((image: Image) => {
                            return image.url;
                        })
                    })
                setVisible(true);
    ...
    const onFinish = async (values: Product) => {
        try {
            setLoading(true);
            await axios.patch(`/api/${params.storeId}/product/${initialData?.id}`, values);
